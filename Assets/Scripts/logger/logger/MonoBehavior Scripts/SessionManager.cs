...
public class SessionManager : MonoBehaviour {
	...
	/// </summary>
	void OnDestroy() {
		Logger.Instance.LevelAborted("Session Being Terminated", SceneManager.GetActiveScene().name /*Application.loadedLevelName */);
	...
	public void LoadLevel(int level) {
		Logger.Instance.LevelEnd(SceneManager.GetActiveScene().name /* Application.loadedLevelName */);
		restarting = false;
	...
	void OnLevelWasLoaded(int level) {
		if(!restarting) {
			Logger.Instance.LevelStart(SceneManager.GetActiveScene().name /* Application.loadedLevelName */);
		}
		
		else {
			Logger.Instance.LevelRestart(SceneManager.GetActiveScene().name /* Application.loadedLevelName */);	
		}
