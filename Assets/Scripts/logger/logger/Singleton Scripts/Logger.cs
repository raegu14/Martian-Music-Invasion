...
#region ==============================|        Private Methods       |==============================
	...
	/// </summary>
	private void writeXML(string selection, string action, string input, string transactionID) {
	...
	private void write(string selection, string action, string input, string transactionID) {            
        if (this.LogWriter == null || !this.InSession) {
			Debug.Log ("Logger.write called with LogWriter == NULL or InSession == False");
            Debug.Log(selection);
            Debug.Log(action);
            Debug.Log(input);
		...
		this.LogWriter.Write(System.String.Format("{{\n  \"Time\": \"{0:yyyy-MM-dd HH:mm:ss.fff}\",\n  \"SessionID\": \"{1}\",\n  \"UserID\": \"{2}\",\n  \"LevelName\": \"{3}\",\n  \"Selection\": \"{4}\",\n  \"Action\": \"{5}\",\n  \"Input\": \"{6}\",\n \"AWT\": \"{7}\"\n, \"Count\": \"{8}\"}},",
			System.DateTime.Now.ToUniversalTime(),
			this.SessionID, this.UserID, this.LoggedLevelName, selection, action, input, HttpWriter.AverageWaitTime, writeCount++));
	}
